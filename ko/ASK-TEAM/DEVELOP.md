...
## 0. 세션 시작 — 식별과 인덱스 (필수, 건너뛰지 말 것)

```bash
python AGENTSPECKIT/askctl.py whoami    # 내 handle/role 확인 (미등록이면 team/에 등록 먼저)
python AGENTSPECKIT/askctl.py index     # 모든 INDEX.md 재생성 후 읽기
```

* `whoami`가 실패하면(미등록 git identity) **작업을 시작하지 않습니다.** `team/<handle>.md`를 먼저 등록하세요(`templates/team-TEMPLATE.md` 복사).
* `index`를 먼저 돌리지 않으면 다른 사람의 in-flight 작업을 못 봐 충돌 검출이 실패합니다.
...
4. `AGENTSPECKIT/workitems/INDEX.md` (in-flight 작업 + 그들의 `touches`)
...
`workitems/INDEX.md`에서 `status: proposed|ready`인 항목을 고릅니다.
...
| 결과 | 의미 | 해야 할 일 |
|---|---|---|
| **STOP** (종료코드 2) | 다른 in-flight workitem과 `contracts` 겹침 | 진행하지 말 것. maintainer에게 직렬화 요청(§7). 계약 변경이면 ADR 경유 |
| **WARN** (종료코드 1) | `modules` 겹침 (잠재 의미 충돌) | `conflicts/CF-*.md` 등재(`templates/CF-TEMPLATE.md`), 상대 owner와 순서 합의 |
| **OK** (종료코드 0) | 독립 | `feat/<WI-id>` 브랜치 생성 후 개발 시작 |
...
5. 자율 판단은 `assumptions/ASM-*.md`로 **새 파일** 생성(공유 `ASSUMPTIONS.md`에 append하지 않음). 기존 가정과 충돌하면 `conflicts/`에 기록.
6. 학습한 비자명한 사실은 `notes/<topic>.md`에 기록(추측은 assumptions로).

**전역 계약을 바꿔야 한다고 판단되면 직접 `ARCHITECTURE.md`를 고치지 마세요.** STOP 사유로 보고 §6 절차를 따릅니다.
...
* **제외**: 생성 `INDEX.md`(git 미추적), `ARCHITECTURE.md`/`PLAN.md`(maintainer), `history/**`(INTEGRATE).
...
`detect`가 STOP이거나 전역 계약 변경이 필요하면:
...
## 수행한 작업 / 변경 파일
## 테스트 결과 (실행 명령 / 통과·실패)
## touches (contracts / modules) 와 detect 결과
