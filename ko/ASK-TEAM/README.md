...
> 기존 Agent-Spec-Kit(1인/순차)의 **자매 프레임워크**입니다. 팀 프롬프트가 인용하는 솔로 절의 참조 사본은 [reference/](reference/README.md)에 동봉되어 있습니다.
...
2. **신규 프로젝트**면 KICKOFF 실행 전에 `AGENTSPECKIT/SOURCES/REQUIREMENTS.md`에 초기 요구사항을 작성합니다 — 이 킷에는 템플릿이 없으므로 솔로 킷의 `SOURCES/REQUIREMENTS.md` 템플릿을 재사용하거나, 목적 / 대상 사용자 / MVP 기능 / 횡단 기준선을 자유 형식으로 적습니다. **기존 코드베이스**(ADOPT)면 선택 사항입니다.
...
각 프롬프트는 에이전트에게 해당 파일을 읽고 따르라고 지시하는 방식으로 실행합니다(예: "AGENTSPECKIT/KICKOFF.md를 읽고 프로젝트를 초기화하세요").
