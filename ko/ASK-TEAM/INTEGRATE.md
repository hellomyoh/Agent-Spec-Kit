...
의미 충돌 재검출, 전역 계약 직렬화, 이력 기록, 인덱스 재생성, 전체 회귀를 담당합니다.
규약은 [CONVENTIONS.md](CONVENTIONS.md)가 우선합니다.

> 이 프롬프트는 `role: maintainer`만 실행합니다. `python AGENTSPECKIT/askctl.py whoami`로 역할을 먼저 확인하세요.
...
```bash
python AGENTSPECKIT/askctl.py whoami    # role: maintainer 확인
python AGENTSPECKIT/askctl.py index     # 인덱스 재생성
```

`role`이 maintainer가 아니면 중단하고 maintainer에게 위임합니다.

---

## 1. 통합 대상 수집

* `workitems/INDEX.md`에서 `status: review`인 workitem과 그 PR/브랜치를 모읍니다.
...
```bash
# 통합 후보 전체에 대해 교차 재검출
python AGENTSPECKIT/askctl.py detect <WI-id>   # 후보마다
```
...
* 생성 `INDEX.md`는 git 미추적이므로 merge 충돌 대상이 아닙니다.
...
```bash
python AGENTSPECKIT/askctl.py index     # workitems/history/assumptions/notes/sources/team INDEX 재생성
```
...
## 8. PROGRESS 다이제스트 갱신 (선택)

`PROGRESS.md`(호환 진입점)가 인덱스를 가리키도록 유지합니다. 활성 workitem/세션 요약을 갱신하되, 진행 상태의 진실은 `workitems/INDEX.md`임을 명시합니다.
