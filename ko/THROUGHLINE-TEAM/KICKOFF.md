# KICKOFF.md — 팀 프로젝트 초기화 (THROUGHLINE Team)
...
루트 3파일(프로젝트 `README.md`·`AGENTS.md`·`CLAUDE.md`)을 제외하고 모두 `THROUGHLINE/` 아래에 만듭니다.
...
* 초기화는 생성한 산출물 전체(루트 3파일 + THROUGHLINE/)를 묶는 **commit으로 끝나며**, 원격이 있으면 push합니다 — 조율층이 공유 브랜치에 올라가야 기여자가 볼 수 있으므로, **이 commit 전에는 claim을 시작할 수 없습니다.**
