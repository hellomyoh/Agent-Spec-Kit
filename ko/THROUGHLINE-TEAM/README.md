...
> THROUGHLINE Solo 에디션(1인·순차)의 **자매 프레임워크**입니다. 팀 프롬프트가 인용하는 솔로 절의 참조 사본은 [reference/](reference/README.md)에 동봉되어 있습니다.
...
## 0. 포지셔닝 — solo THROUGHLINE의 자매

기존 THROUGHLINE(`THROUGHLINE/`)는 **1인/순차·자율 개발**을 위해 `PLAN.md`·`PROGRESS.md`·`HISTORY.md`·`ASSUMPTIONS.md`·`NOTES.md`·`SOURCES/INDEX.md` 같은 **전역 단일 파일**을 중심으로 동작합니다. 단순하지만, N명이 동시에 작업하면 이 파일들이 충돌 핫스팟이 됩니다.

THROUGHLINE Team은 이 문제를 **티어로 점진 도입하지 않고**, 처음부터 팀-우선으로 모든 메커니즘을 확정한 별도 프레임워크입니다. 1인 개발이라면 기존 THROUGHLINE을 쓰는 것이 더 가볍습니다. THROUGHLINE Team은 **실제로 N명이 동시에 개발하는 팀**을 위한 것입니다.
...
THROUGHLINE Team의 모든 설계 결정은 한 가지 사실에서 나옵니다.
...
| 층 | 내용 | 사는 곳 |
|---|---|---|
| **조율층(coordination)** | workitem 선언(`touches`), conflicts, 페르소나 인스턴스, 전역 계약(ARCHITECTURE/PLAN) | **공유 브랜치 — claim 시 즉시 published** |
...
프로젝트에 적용하면 모든 산출물은 `THROUGHLINE/` 아래에 생성됩니다(루트 3파일 예외: 프로젝트 `README.md`·`AGENTS.md`·`CLAUDE.md`). 기존 THROUGHLINE과 동일한 루트 규약입니다.
...
THROUGHLINE/
  ...
  SOURCES/
    ...
    REQUIREMENTS.meta.md         # 그 triage meta (id: REQUIREMENTS)
  ...
  reference/                     # 동봉된 솔로 킷 참조 사본 (형식 참조 전용 — reference/README.md)
```
...
- **SoT = 각 항목 파일의 frontmatter.** 단일 작성자가 자기 파일만 씁니다(`WI-*.md`, `ASM-*.md`, `SRC-*.meta.md`, `HIST-*.md`, `team/*.md`, `CF-*.md`, `personas/*.md`).
...
```text
git fetch 후, 최신 공유 브랜치의 workitems/*.md 중 status ∈ {claimed, in_progress}를 읽어 (CONVENTIONS §4.5)
내 touches와 교차한다.
...
2. 분해 / claim   workitems/WI-*.md 작성(touches) → 공유 브랜치 커밋·push → 검출(§6)
...
**원자 커밋(재정의):** feature 브랜치의 원자 커밋 = **코드 + 그 workitem의 작업층 파일**. ARCHITECTURE/PLAN(maintainer), history(INTEGRATE), `workitems/WI-*.md`(조율층 — 상태 변경은 공유 브랜치에서 커밋·push, CONVENTIONS §4.5)는 **포함하지 않습니다**. "코드와 대응 문서를 한 커밋에"라는 원칙은 *workitem 스코프 내에서* 유지됩니다.

---

## 8. 빠른 시작

1. 이 저장소를 clone하고 `ko/THROUGHLINE-TEAM/`의 내용을 프로젝트 루트의 `THROUGHLINE/`로 복사합니다.
2. **신규 프로젝트**면 KICKOFF 실행 전에 `THROUGHLINE/SOURCES/REQUIREMENTS.md`에 초기 요구사항을 작성합니다 — 이 킷에는 템플릿이 없으므로 솔로 킷의 `SOURCES/REQUIREMENTS.md` 템플릿을 재사용하거나, 목적 / 대상 사용자 / MVP 기능 / 횡단 기준선을 자유 형식으로 적습니다. **기존 코드베이스**(ADOPT)면 선택 사항입니다.
3. **Maintainer가** `team/<handle>.md`를 본인 것부터 등록하고 `role: maintainer`로 둡니다(`templates/team-TEMPLATE.md` 복사). 각 기여자도 자기 `team/<handle>.md`를 등록합니다.
4. 기여자는 `DEVELOP.md`, maintainer는 `INTEGRATE.md` 프롬프트로 작업합니다. 세션 시작 시 에이전트가 신원(§4)을 확인하고 필요한 항목 파일을 직접 읽습니다 — 별도 명령 입력이 없습니다.

### 언제 어떤 프롬프트를 쓰나 (사용법 맵)

각 프롬프트는 에이전트에게 해당 파일을 읽고 따르라고 지시하는 방식으로 실행합니다(예: "THROUGHLINE/KICKOFF.md를 읽고 프로젝트를 초기화하세요").

| 상황 | 실행 | 누가 |
|---|---|---|
| 신규 팀 프로젝트 초기화 (요구사항 → 구조 + 초기 workitem) | [KICKOFF.md](KICKOFF.md) | maintainer |
| 기존 코드베이스에 적용 (as-built 역문서화) | [ADOPT.md](ADOPT.md) | maintainer |
| 개발 시작 / 이어서 개발 (claim → 검출 → 구현 → PR) | [DEVELOP.md](DEVELOP.md) | contributor |
| review 완료 workitem 통합 (재검출 → 직렬화 → merge → history) | [INTEGRATE.md](INTEGRATE.md) | maintainer |
| 주기적 표류·조율 무결성 감사 (Phase 완료 / 릴리즈 / 표류 의심) | [AUDIT.md](AUDIT.md) | maintainer |
| 규칙 찾아보기 (파일 등급 · 식별 · 충돌 · 공유 브랜치 · 커밋) | [CONVENTIONS.md](CONVENTIONS.md) | 전원 |
| frontmatter 형식 찾아보기 | [SCHEMAS.md](SCHEMAS.md) · [templates/](templates/) | 전원 |
| 솔로 킷에서 인용하는 상세 형식 (feature / QA / 페르소나 / 감사) | [reference/](reference/README.md) | 전원 |

---

## 9. solo THROUGHLINE과의 차이

| 항목 | Solo 에디션 | THROUGHLINE Team |
|---|---|---|
...
2. **검출 ≠ 강제.** `touches` 미선언/오선언이면 검출이 실패합니다. 에이전트가 규약을 따른다는 보장에 의존하며(프롬프트는 강제가 아님 — THROUGHLINE 저장소 가이드 README의 "정직한 잔여 한계" 절 참조), INTEGRATE 재검출이 마지막 그물이지만 사후입니다. 진짜 *강제*가 필요하면 git 플랫폼 계층(protected branch / CI)을 옵션으로 얹습니다.
3. **비용은 의도된 대가.** 세션당 오버헤드(검출 시 in-flight workitem frontmatter 읽기)와 maintainer의 INTEGRATE 부담을 수용합니다. `workitems/archive/`(CONVENTIONS §9)가 이 세션당 스캔을 프로젝트 누적 workitem 수가 아니라 활성 백로그로 제한해주지만, INTEGRATE 재검출과 maintainer 부담은 여전히 동시 in-flight 작업량에 비례합니다. 1인이라면 solo THROUGHLINE이 더 쌉니다.
...
| `reference/` | 동봉된 솔로 킷 원문 사본(SOLO-KICKOFF·DEVELOPINIT·ADOPT·AUDIT) — 인용 형식 참조 전용, 충돌 시 팀 킷 우선 |
