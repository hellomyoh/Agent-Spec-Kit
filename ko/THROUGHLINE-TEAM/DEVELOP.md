...
THROUGHLINE Team 팀 개발에서 **한 명의 기여자(인간/AI 에이전트)가 하나의 workitem을 수행**하는 절차입니다.
...
> 경로 기준: 산출물은 모두 `THROUGHLINE/` 아래에 있습니다(루트 3파일 예외).
...
1. `AGENTS.md` (프로젝트 루트)
2. `THROUGHLINE/ARCHITECTURE.md` (횡단 계약 — 항상 로드)
3. `THROUGHLINE/PLAN.md` (로드맵)
4. `THROUGHLINE/workitems/*.md` 중 **in-flight 항목의 frontmatter** (현황 + 다른 사람의 `touches`)
...
2. `features/*.md`의 명세와 `ARCHITECTURE.md` 계약을 확인합니다(없으면 명세부터, 비자명 기능은 `personas/`+`discussion/` 리뷰 — Solo 에디션 DEVELOPINIT §6 방식, [reference/SOLO-DEVELOPINIT.md](reference/SOLO-DEVELOPINIT.md)).
