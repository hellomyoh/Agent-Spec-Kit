# Solo 적합성 파일럿 (ASK / AGENTSPECKIT)

solo ASK 프롬프트가 명세대로 구동되는지 1회 확인합니다. 각 단계 실행 후 **체크 명령**을 돌려 PASS/FAIL을 기입하세요.
`<REPO>` = 이 저장소 루트.
...
```bash
mkdir -p /tmp/ask-solo && cd /tmp/ask-solo && git init -q
git config user.email dev@example.com && git config user.name dev
cp -r "<REPO>/ko/AGENTSPECKIT" ./AGENTSPECKIT
cp "<REPO>/tests/conformance/seeds/REQUIREMENTS.md" AGENTSPECKIT/SOURCES/REQUIREMENTS.md
git add -A && git commit -qm "chore: seed ASK kit + requirements"
git rev-parse HEAD > /tmp/ask-solo-base   # 기준 커밋 기록
```
...
for f in ARCHITECTURE.md PLAN.md PROGRESS.md HISTORY.md ASSUMPTIONS.md NOTES.md TODO.md; do
  test -f "AGENTSPECKIT/$f" && echo "C1 ok $f" || echo "C1 FAIL $f"
done
# C2 기능명세 + 인덱스
...
# C5 PROGRESS 다음 명령
grep -iE '다음 세션 첫 명령|first command' AGENTSPECKIT/PROGRESS.md && echo "C5 ok" || echo "C5 FAIL"
# C6 초기화가 commit 됨
...
| C3 | `AGENTSPECKIT/`·루트3파일 밖 산출물 없음 | ☐ |
...
grep -E '^## \[[0-9]{4}-[0-9]{2}-[0-9]{2}\] (init|adopt|feat|fix|docs|test|qa|audit|chore) \|' AGENTSPECKIT/HISTORY.md \
  && echo "C7 ok" || echo "C7 FAIL"
# C8 원자 커밋: 한 커밋에 코드 + AGENTSPECKIT 문서가 함께
git show --name-only --pretty=format: "$DEV" | grep -q 'AGENTSPECKIT/' \
  && git show --name-only --pretty=format: "$DEV" | grep -qvE 'AGENTSPECKIT/|^$' \
  && echo "C8 ok" || echo "C8 FAIL (코드+문서 동시 커밋 아님)"
# C9 테스트 실제 실행 흔적 (실행 명령 + 통과/실패 기록)
grep -iE '실행 명령|(npm|pytest|go test|cargo|jest|vitest).*' AGENTSPECKIT/HISTORY.md \
  && echo "C9 ok(명령 기록 확인)" || echo "C9 FAIL(테스트 실행 명령 미기록)"
# C10 PROGRESS '다음 명령'이 KICKOFF 대비 갱신됨
git log -p -- AGENTSPECKIT/PROGRESS.md | grep -iE '^\+.*다음.*명령' \
...
printf '# Feature: Orphan (파일럿 주입)\n' > AGENTSPECKIT/features/feature-orphan.md
...
grep -E '^## \[[0-9-]+\] audit \|' AGENTSPECKIT/HISTORY.md && echo "C12 ok" || echo "C12 FAIL"
```
