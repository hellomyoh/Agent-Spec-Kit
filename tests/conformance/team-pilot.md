# Team 적합성 파일럿 (ASK-Team)

ASK-Team 프롬프트가 명세대로 구동되는지 1회 확인합니다. 핵심은 **충돌 검출(touches)·식별(owner==author)·고정 INDEX 부재·single-writer·INTEGRATE 이력**입니다.
...
cp -r "<REPO>/ko/ASK-TEAM" ./AGENTSPECKIT
cp "<REPO>/tests/conformance/seeds/REQUIREMENTS.md" AGENTSPECKIT/SOURCES/REQUIREMENTS.md
mkdir -p AGENTSPECKIT/team
cat > AGENTSPECKIT/team/alice.md <<'EOF'
---
handle: alice
name: Alice
...
---
EOF
cat > AGENTSPECKIT/team/bob.md <<'EOF'
---
handle: bob
name: Bob
...
---
EOF
git add -A && git commit -qm "chore: seed ASK-Team kit + team registry + requirements"
...
ASK-Team `KICKOFF.md`로 팀 초기화를 수행하게 합니다(초기 workitem을 `touches` 포함해 분해).

```bash
cd /tmp/ask-team
# C1 팀 구조 디렉토리
for d in workitems conflicts team sessions history assumptions notes SOURCES; do
  test -d "AGENTSPECKIT/$d" && echo "C1 ok $d" || echo "C1 FAIL $d"
done
# C2 고정 INDEX·askctl·python 부재
{ git ls-files | grep -E 'AGENTSPECKIT/.*INDEX\.md' && echo "C2 FAIL (INDEX 커밋됨)"; } || echo "C2 ok (INDEX 없음)"
test ! -f AGENTSPECKIT/askctl.py && ! grep -rql askctl AGENTSPECKIT && echo "C2b ok (askctl 없음)" || echo "C2b FAIL"
# C3 초기 workitem ≥2, 모두 touches 보유
n=$(ls AGENTSPECKIT/workitems/WI-*.md 2>/dev/null | wc -l); echo "workitems=$n"
miss=0; for f in AGENTSPECKIT/workitems/WI-*.md; do grep -q 'touches:' "$f" || { echo "no touches: $f"; miss=1; }; done
test "$n" -ge 2 -a "$miss" -eq 0 && echo "C3 ok" || echo "C3 FAIL"
# C4 REQUIREMENTS 동결
grep -riE '(applied|반영 완료)' AGENTSPECKIT/SOURCES/SRC-*requirements*.meta.md AGENTSPECKIT/SOURCES/*.meta.md 2>/dev/null \
  && echo "C4 ok" || echo "C4 FAIL"
# C5 신원: 현재 git email이 team 레지스트리에 존재
grep -rl "$(git config user.email)" AGENTSPECKIT/team/ && echo "C5 ok" || echo "C5 FAIL"
```
...
git log --all --oneline -- AGENTSPECKIT/ARCHITECTURE.md
# 6b modules 겹침 → conflicts/CF 생성
ls AGENTSPECKIT/conflicts/CF-*.md >/dev/null 2>&1 && echo "6b ok (CF 생성)" || echo "6b FAIL (CF 없음)"
...
{ git ls-files | grep -E 'AGENTSPECKIT/.*INDEX\.md' && echo "C9 FAIL"; } || echo "C9 ok"
# C10 SRC meta applied 처리(반영된 source)
grep -rilE '(applied|반영 완료)' AGENTSPECKIT/SOURCES/*.meta.md && echo "C10 ok" || echo "C10 (반영 source 없으면 N/A)"
# C11 single-writer: ARCHITECTURE.md를 건드린 커밋 작성자가 maintainer뿐
echo "ARCHITECTURE 작성자:"; git log --format='%ae' -- AGENTSPECKIT/ARCHITECTURE.md | sort -u
...
printf '\n<!-- pilot: contributor 직접 수정 (위반) -->\n' >> AGENTSPECKIT/ARCHITECTURE.md
git -c user.email=bob@example.com -c user.name=bob commit -aqm "test: contributor edits ARCHITECTURE (pilot violation)"
```

ASK-Team `AUDIT.md`를 실행하게 합니다. 이후:

```bash
cd /tmp/ask-team
# C13 AUDIT가 audit 이력 이벤트 기록
ls AGENTSPECKIT/history/**/HIST-*audit*.md >/dev/null 2>&1 \
  || grep -rlE 'audit' AGENTSPECKIT/history/ >/dev/null 2>&1 && echo "C13 ok" || echo "C13 FAIL"
```
...
모든 MUST PASS → **team 적합**. 특히 **C6a(STOP)·C7(owner==author)·C9(INDEX 미커밋)·C11(single-writer)** 이 ASK-Team의 핵심 차별 메커니즘이므로 우선 확인합니다.
