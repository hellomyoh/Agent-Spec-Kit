...
class Catalog:
    ...
    def search(
        ...
        is a non-blank string, items are kept only when it is a case-insensitive
        substring of the item's 'name'. A `query` that is None, empty, or only
        whitespace applies no text filter, so the search behaves like a plain
        list view and all items pass through (subject to any `where`/`sort_by`/
        `limit`).
        ...
        the matching value (multiple fields are combined with AND). A `where`
        field that no item carries simply matches nothing, so the result is an
        empty list (an unknown field is not an error).
