...
> **inside the `AGENTSPECKIT/` folder**. Every artifact path in this document is relative to `AGENTSPECKIT/`.
...
> **No re-adoption:** A project where the `AGENTSPECKIT/` folder already exists and contains artifacts is **a project that has already been adopted**.
> Do not run ADOPT again; check `AGENTSPECKIT/PROGRESS.md` to continue, or confirm with the user.
...
└── AGENTSPECKIT/        # ★ All framework artifacts (isolated from existing project folders)
...
1. **Inventory existing artifacts.** First, check whether existing artifacts are present in `AGENTSPECKIT/`
...
18. **Report adoption completion** (Section 7 format).

---

# 5. Existing-file merge rules (important)

Since the kit's artifacts are isolated inside AGENTSPECKIT/, the merge targets are effectively just the **four root files**
(`README.md`, `AGENTS.md`, `CLAUDE.md`, `.gitignore`). If these already exist, **do not overwrite them.**
Same-named folders such as the existing project's `docs/` are separate from the kit's artifacts (AGENTSPECKIT/docs/), so do not merge or modify them.
