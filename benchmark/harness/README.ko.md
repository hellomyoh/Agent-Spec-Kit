# THROUGHLINE-QBench 하네스 (B3 Revert-to-Origin)

🌐 [English](README.md) · **한국어**

유지보수자측 평가 도구. THROUGHLINE 런타임 키트(`en/`,`ko/`)와 분리됨. 방법론: [../METHODOLOGY.ko.md](../METHODOLOGY.ko.md).
...
## B3 실행 절차 (요약)
1. **Base→R1→R2 체인 생성**: 각 그룹(L0/L1/L-SSOT)이 step0→1→2를 순차 개발(이전 step 코드 + 그 그룹의 기억 인계). L-SSOT는 SPEC+HISTORY(append-only)+PROGRESS 유지.
...
신규 실행 결과: [../results/B3_results.json](../results/B3_results.json) (L0 0/2, L1 2/2, L-SSOT 2/2).
