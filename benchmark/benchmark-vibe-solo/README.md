...
Implements `BENCHMARK-PLAN-VIBE-SOLO.md` (in this folder) as a runnable file structure.

**Hypothesis.** In vibe-coding, users give incomplete/inconsistent instructions and forget their own
past intent. throughline-solo (feature spec + SSOT docs + a conflict-handling discipline) should beat plain
...
- **3 prompt levels** × **2 modes** × **7 sessions (s0–s6)**.
  ...
  - Modes: `baseline-general` (no SSOT, just build the request) vs `throughline-solo` (maintain SSOT + run a
...
- **The two conflicts** (plan §4.5):
  - **S5 — real intentional change** (explicit in all levels): unknown `where` field: *raise* → *ignore*.
    Correct = adopt (both modes). Tests that throughline-solo doesn't *stubbornly* keep an old decision.
  - **S6 — forgetting / ambiguous drift** (the discriminator): "show everything when the search box is
    ...
    **adopt** blank→all (the user explicitly, knowingly overrides). throughline-solo's SSOT records the S3
...
benchmark-vibe-solo/
  ...
  agents/                         # dev-agent prompt templates per mode + judge instructions
    dev_baseline-general.md  dev_throughline-solo.md  judge_instructions.md
  tasks/catalog/
...
```
cd benchmark/benchmark-vibe-solo
PYTHONIOENCODING=utf-8 python eval/_selftest.py          # Stage -1 gate (must pass)

# for each mode in {baseline-general, throughline-solo}, level in {beginner,intermediate,advanced}, n in 0..6:
...
Can: whether throughline-solo helps on sparse/ambiguous prompts, whether it curbs intent-drift, at which user
level it pays off, whether its doc cost is offset. Cannot: that THROUGHLINE wins on all tasks, Team/multi-agent
