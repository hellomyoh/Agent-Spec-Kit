...
- S6: blank search box now shows everything (REVERSES S3). A non-None `query`
  that is empty or whitespace-only no longer returns `[]`; it skips the name
  filter so every item flows through (then `where`/`sort_by`/`limit` still
  apply), behaving like a plain list. Implemented as an `if needle:` guard around
  the name-filter step instead of an early `return []`. `query=None` list mode,
  non-blank name search, `where`, `sort_by`, and `limit` are all unchanged.
