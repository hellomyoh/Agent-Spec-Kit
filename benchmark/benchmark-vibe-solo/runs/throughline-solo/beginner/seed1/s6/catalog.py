...
Behaviour policies that keep results predictable:
- An empty search shows the whole catalog. With no `where` and a blank or
  omitted `query`, `search` applies no filter and returns every item (like an
  ordinary list), the same items `all()` would return. A real criterion
  (`where`, or a non-blank `query`) narrows it down.
- `where` (field equality filter) is forgiving: filtering on a field that no
  item has simply yields no matches (an empty list); it does not raise.
- `sort_by` still fails loudly: sorting on a field that no item has raises
  KeyError, so a typo'd sort field is caught instead of doing something
  surprising.
...
class Catalog:
    ...
    def all(self) -> list[dict]:
        ...
        The explicit "view the full list" affordance. A criterion-less `search`
        now returns the same items, but `all()` remains the direct, unfiltered
        way to list everything.
        """
    ...
    def search(
        ...
        Empty search shows everything: with no `where` and a None or blank
        (empty or whitespace-only) `query`, no filter is applied and every item
        is returned, in insertion order — an ordinary list. Supplying a real
        criterion narrows the result.
        ...
        'ap' matches 'Apple'). A blank `query` applies no text filter.

        `where` (field equality filter): items are kept only when every field in
        `where` equals the item's value for that field. An item that lacks a
        given field does not match it. Filtering on a field that no item has is
        not an error: it simply yields no matches (an empty list). When both
        ...
        KeyError; on an empty result there is nothing to order.
        ...
        returns all of it.
        """
        # A blank or omitted query applies no text filter; with no `where`
        # either, the whole catalog passes through (an ordinary list).
        query_is_blank = query is None or query.strip() == ""

        items = self._items

        if where is not None:
            # Forgiving filter: an item must equal every `where` field. An item
            # lacking a field (.get -> None) does not match a non-None value, so
            # filtering on a field no item has simply yields no matches.
            items = [
                item
                for item in items
