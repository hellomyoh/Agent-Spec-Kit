...
class Catalog:
    ...
    def search(self,
        ...
        query:
            ...
            A blank or whitespace-only string ("" or "   ") -> list mode too
            (every item): a blank query is the list view, not a filter.
        ...
        where:
            ...
            An item that lacks a given key simply does not match (it is filtered
            out), so a key absent from every item yields no results ([]). Empty
            {} imposes no constraint and lists all items.
        sort_by:
        ...
        # query=None and a blank/whitespace-only query are both list mode (the
        # blank query is the list view). Only real search text filters by name.
        ...
        # An item that lacks a where key does not match that constraint, so it is
        # filtered out (missing key -> not equal). A key absent from every item
        # therefore matches nothing and the result is [].
