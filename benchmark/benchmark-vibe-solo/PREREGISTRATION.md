# PREREGISTRATION — Vibe Solo Benchmark (seed1 pilot)

Fixed BEFORE running. Prompt levels are assigned in advance and never reclassified from results (plan §3).

## Hypotheses (plan §8)
- **H1** Beginner prompts: throughline-solo ≥ baseline-general on goal attainment / fewer invariant violations.
- **H2** Intermediate: throughline-solo retains some advantage.
- **H3** Advanced: advantage shrinks; throughline-solo's doc cost may not pay off (acceptable).
- **H4** Higher ambiguity ⇒ more baseline drift. **H5** throughline-solo dampens that drift.
- **H6** throughline-solo's extra questions (if any) reduce rework/regression rather than just adding burden.
...
- Factors: mode {baseline-general, throughline-solo} × level {beginner, intermediate, advanced} × session s0..s6.
...
1. **S6 conflict (the headline):** at **beginner & intermediate**, `throughline-solo` keeps `blank_query` correct
   (preserve `[]`) while `baseline-general` is expected to drift (blank→all → `blank_query` fail + regression).
   At **advanced**, both should adopt blank→all (no throughline-solo advantage expected).
2. **Late-session invariant/regression:** at beginner/intermediate, `throughline-solo` invariant_violations and
...
3. **S5 control:** both modes adopt the explicit change (`unknown_where_ignored` passes at s5+ for both);
   throughline-solo must NOT stubbornly keep raising. (Guards against "throughline-solo just always keeps old behavior".)
4. **Cost:** throughline-solo token/turn cost is recorded; the claim requires its quality gain to offset added cost.

## Seed1 pilot = go/no-go, not inference
Single seed; we read DIRECTION + harness soundness, not p-values (plan §9). Interpretation:
- **Go**: at beginner/intermediate, throughline-solo preserves the S6 policy / fewer violations while baseline
...
- **Revise THROUGHLINE docs**: throughline-solo writes docs but still drifts at s6 ⇒ SSOT structure/authority unclear.
- **Stop or narrow claim**: difference only absent at advanced but present at beginner/intermediate ⇒
  narrow THROUGHLINE's claimed value to "beginner/intermediate user correction" (plan §10 Stop/narrow).

## Analysis questions (answer in RESULTS)
Plan §8 Q1–Q7. Minimum: does ambiguity (level) modulate the throughline-solo − baseline gap, concentrated at S6?
