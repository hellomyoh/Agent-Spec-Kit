"""Vibe Solo Benchmark — driver (orchestrator-driven, like benchmark-realapp).
...
def prepare(mode, level, n):
    ...
    if mode == "throughline-solo":
        ...
        mem = (f"MODE=throughline-solo. Your SSOT docs are in docs/ssot/ ({', '.join(SSOT_DOCS)}). "
               "READ them before coding; UPDATE them after. Follow agents/dev_throughline-solo.md: if the user's "
