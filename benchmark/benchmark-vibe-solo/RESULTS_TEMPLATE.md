# RESULTS — Vibe Solo Benchmark (seed1 pilot)  [TEMPLATE — fill after running]
...
| throughline-solo/beginner | | | | | | | |
| baseline-general/intermediate | | | | | | | |
| throughline-solo/intermediate | | | | | | | |
| baseline-general/advanced | | | | | | | |
| throughline-solo/advanced | | | | | | | |
...
| level | baseline-general S6 | throughline-solo S6 | oracle-correct? | throughline-solo detected conflict? |
|---|---|---|---|---|
| beginner | | | | |
| intermediate | | | | |
| advanced | | | | |
...
Did BOTH modes adopt unknown-field→ignore at s5+? (throughline-solo must not stubbornly keep raising.) ____

## 5. Doc & process scores (judge)
- Doc totals (0–15) per mode/level: ____
- Process totals (esp. S5/S6) per mode/level: ____
- Did throughline-solo's SSOT actually carry the S3 blank policy into S6? ____

## 6. Cost
- tokens / turns / tool calls / edit_churn per mode (esp. throughline-solo overhead vs baseline): ____
- Composite (`eval/judge.py aggregate`): paste table.

## 7. Answers to plan §8 questions
Q1 beginner THROUGHLINE gain? Q2 intermediate? Q3 advanced cost vs gain? Q4 ambiguity→drift?
Q5 THROUGHLINE dampens drift? Q6 questions reduced rework? Q7 doc quality→fewer late violations?

## 8. Go / No-Go verdict (PREREGISTRATION §)
[ ] Go  [ ] Revise prompts  [ ] Revise task  [ ] Revise THROUGHLINE docs  [ ] Stop/narrow claim — because: ____
