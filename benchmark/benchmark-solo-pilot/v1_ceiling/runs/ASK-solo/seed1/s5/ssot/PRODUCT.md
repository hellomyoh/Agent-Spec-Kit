...
- S1 complete: `query(where=...)` equality filter (AND across all key==value
  pairs). Unknown-field handling was CHANGED in S5 (see below): originally an
  unknown field raised `KeyError` (D6); it now yields no match instead (D10).
...
- S5 complete: CHANGED `where` unknown-field handling — a `where` field absent
  from every stored record now yields NO match (`query()` returns `[]`) instead
  of raising `KeyError`; empty store + non-empty `where` also returns `[]` (D10,
  supersedes D6's raise rule). `sort_by`'s `KeyError` on unknown field (D7) is
  unchanged.
- query() now implements its full pinned parameter set (where, sort_by, limit,
  offset, select).
