...
filters records by an optional `where` equality match, then returns the
survivors in insertion order with pagination via `offset`/`limit`. When
`limit` is omitted, a default page size of 25 records applies.
"""
...
class Store:
    ...
    def query(self,
        ...
        """Return matching records as copies, in insertion order, paginated.
        ...
        filter is applied first. `offset` then skips that many leading records
        (default 0), and `limit` caps the number returned; when `limit` is
        omitted, the default page size of 25 applies.
        """
        results = self.all()
        if where:
