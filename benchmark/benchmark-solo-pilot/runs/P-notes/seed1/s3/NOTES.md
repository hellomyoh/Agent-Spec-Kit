...
## State after S3 (Adjust default page size)
...
- `query(where=None, sort_by=None, limit=None, offset=0)` pipeline so far:
  ...
  - **Pagination (S1):** `offset` skips N leading records of the FILTERED set;
    `limit` caps count. **DEFAULT_PAGE_SIZE = 25** (S3, was 7 in S1) when
    `limit is None`. `limit=0` returns []. Distinguish `limit is None` (->25)
    from `limit=0`. Default applies to the POST-filter result set.
...
- Full intended pipeline once all built:
  ...
  Insert `sort` AFTER filter and BEFORE the offset/limit slicing so the default
...
- Verified S3 with `python -c "import miniquery"` + check DEFAULT_PAGE_SIZE==25
  and that omitted limit returns up to 25. S0/S1/S2 checks previously passed.
