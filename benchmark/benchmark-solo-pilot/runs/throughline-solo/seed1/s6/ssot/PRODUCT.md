...
- S4: `sort_by` — ascending, stable (ties keep insertion order), applied after `where`
  and before paging. Full `query()` surface is now implemented.
- S5: default page size set to `DEFAULT_PAGE_SIZE = 40` (was 25).
- S6: default page size restored to `DEFAULT_PAGE_SIZE = 7` — the original S1 value
  (was 40); reverts the S3/S5 adjustments.
