...
## S6 — Restore the original default page size (done)
Restored `DEFAULT_PAGE_SIZE` to `7` — the value it was originally given when first
introduced in S1 (see D10, supersedes D9). This undoes the later adjustments (25 in S3,
40 in S5) and returns the omitted-`limit` cap to its original value; explicit `limit`
(including 0) still overrides it. No other behaviour changed.
- Verified `python -c "import miniquery"` imports cleanly and
  `miniquery.DEFAULT_PAGE_SIZE == 7`.
