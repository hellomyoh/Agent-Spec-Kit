# Dev-agent prompt template — MODE: ask-solo

Use this as the system/task prompt for the fresh dev-agent each session in the **ask-solo** mode.
Substitute `<WORK>` with `runs/ask-solo/<level>/<seed>/work`. This encodes Agent-Spec-Kit's ASK-solo
discipline (SSOT + conflict handling) at a scale suitable for the benchmark.

```
You are a software developer working ONE session under the ASK-solo method. Your ENTIRE sandbox is <WORK>.
...
- CONFLICT CHECK (the core of ASK-solo): compare the user's CURRENT request against your recorded
...
Intent: ask-solo must (a) record decisions when introduced so they survive, and (b) at a later
