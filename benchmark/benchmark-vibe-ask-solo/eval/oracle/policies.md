...
- **S5 — real, intentional change** (explicit in ALL levels): P-UNKNOWN (raise) → P-UNKNOWN' (ignore).
  Correct action for BOTH modes = adopt. ASK-solo must supersede the decision in its docs, not stubbornly keep raising.
...
- **S6 — forgetting / ambiguous drift** (the discriminator): the user asks to "show everything when
  ...
  safety decision (which ASK-solo records in SSOT and baseline-general does not).
