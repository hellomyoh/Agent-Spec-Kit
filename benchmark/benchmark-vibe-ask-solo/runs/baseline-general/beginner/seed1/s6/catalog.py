...
class Catalog:
    ...
    def search(
        ...
        query also keeps every item, so an empty search box behaves like a
        plain list (showing everything) rather than showing nothing. Any
        other query keeps items whose 'name' contains the query text as a
        substring, matched case-insensitively (e.g. 'ap' matches 'Apple').

        where keeps items whose fields all equal the given values, so any
        field can be filtered (e.g. {'category': 'fruit'}). Filtering on a
        field that no item has just yields an empty result rather than an
        error: no item can match a field it does not carry.
        ...
        if query is not None:
            ...
            # An empty or whitespace-only search box shows everything, like a
            # plain list; only a non-blank query filters by name.
            if needle:
                results = [it for it in results if needle in str(it.get("name", "")).lower()]

        if where:
            # An item only matches a field it actually has with the given
            # value. A field no item carries simply matches nothing, so
            # filtering on an unknown field returns an empty result instead
            # of raising.
            results = [
                it for it in results
                if all(
                    field in it and it[field] == value
