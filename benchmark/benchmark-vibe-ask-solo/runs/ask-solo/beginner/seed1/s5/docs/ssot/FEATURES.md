...
- `limit` (max results): caps how many items are returned — the "top N" of the
  ...
  `TypeError` (a malformed cap is a caller mistake — fail loudly, like a
  `sort_by` typo; this is about a bad cap value, not a missing field, so the S5
  `where` change does not touch it). `bool` is rejected as non-integer (it is
