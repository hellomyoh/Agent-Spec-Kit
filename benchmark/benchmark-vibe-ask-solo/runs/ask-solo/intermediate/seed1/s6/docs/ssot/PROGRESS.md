...
## S6 (seed1) — DONE
- Request (KO): "빈 검색이 평범한 목록 뷰처럼 전체 아이템을 보여주게 해줘. 기존
  search/sort/limit 테스트는 깨지면 안 돼." = make an **empty search** show all items
  like a plain list view; don't break existing search/sort/limit tests.
- Conflict check: **"empty search" is ambiguous** and partly conflicts with **D8**.
  - Reading A (`query=None` / no-arg `search()` — "no search term"): already returns
    all items (D3). Reaffirmed explicitly as **D12** ("empty search = list view").
  - Reading B (`query=""`/whitespace — empty search *string*): currently returns `[]`
    by **D8**, a deliberate S3 decision. Reading B would reverse D8.
  - Action: this is forgetting/ambiguous drift against a recorded policy, **not** a
    stated intentional supersede — so **preserve D8 in code** (blank string → `[]`)
    and **record the conflict as CR1** for user confirmation. Reversing D8 would also
    break the existing S3 blank-string test, violating the prompt's own "don't break
    existing tests" constraint. The "list view shows all" intent is fully met by
    Reading A (D12) without touching D8.
- Behaviour change: none. D12 only names existing D3 behaviour; D8 and all prior
  policies (D2 copy-safety, D5 ci substring, D9 sort, D10 limit, D11 where) preserved.
- Verified via `python -c "import catalog"` plus a 22-check smoke test (no-arg search
  == all, query=None → all, blank string → [] per D8, ci substring, sort ascending +
  stable ties, limit cap / 0 / <0 / None / >len, where present + multi-key AND +
  missing-field → [], query+where AND, limit-after-sort, copy-safety). ALL PASS.
- SSOT updated: FEATURES (empty-search = list view; blank-string distinct + CR1 note),
  DECISIONS (D12; CR1 conflict; D8 annotated, preserved not superseded), PROGRESS.

## Open / deferred
- **CR1 (S6)** — Awaiting user confirmation: should a blank search **string**
  (`query=""`/whitespace) also return all items (reversing D8), or stay as `[]`?
  Currently preserved as `[]` (D8). See DECISIONS CR1.
