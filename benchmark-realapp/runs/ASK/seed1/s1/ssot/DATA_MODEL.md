...
| title      | str                                               | yes      |                                      |
| scope      | str                                               | no       | free-text scope description; None if omitted |
| owner      | int                                               | no       | user_id responsible; None if omitted |
...
| priority   | int                                               | no       | task priority; None if unset         |
| created_by | int                                               | yes      | user_id who created the task         |

### Required columns (DB-enforced)
`project_id`, `org_id`, `title`, `status`, `created_by` — all others nullable.
