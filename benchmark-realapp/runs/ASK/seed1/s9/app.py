"""OpsBoard — app module (Session S1: create & edit tasks).
...
def _define_schema(db):
    ...
    db.define_table(
        "tasks",
        {
            ...
            "priority":   "int",          # task priority (nullable, 0 = unset)
            "created_by": "int",          # user_id
        },
        required=["project_id", "org_id", "title", "status", "created_by"],
    )
...
class Backend:
    ...
    def _is_privileged(self, actor):
        """Legacy check kept for schedule_task. approve/reject use _can_approve instead."""
        return actor.get("role") in ("org_admin", "project_owner")

    def _can_approve(self, actor, task) -> bool:
        """Return True if actor may approve/reject this specific task.

        Rules (S3):
          - org_admin: always allowed (within their org).
          - project_owner: allowed only if they own the task's project.
        ...
        if role == "project_owner":
            # Must own the project the task belongs to
            proj = self._db.get("projects", task["project_id"])
            if proj is not None and proj["owner_id"] == actor.get("user_id"):
                return True
        return False
    ...
    def _create_task(self, actor, payload):
        ...
        scope      = payload.get("scope", "")   # optional; default empty string
        owner      = payload.get("owner", None) # optional; default None
        ...
        if owner is not None and not isinstance(owner, int):
            raise RuntimeError("owner must be int user_id if provided")
        if priority is not None and not isinstance(priority, int):
            raise RuntimeError("priority must be int if provided")
        ...
        task_id = self._db.insert("tasks", row)
    ...
    # Statuses after which core fields are locked (approved and any later status)
    _LOCKED_STATUSES = {"approved", "scheduled", "completed"}
    # Core fields that become immutable once a task is approved or later
    _CORE_FIELDS = {"title", "scope", "owner"}
    ...
    def _update_task(self, actor, payload):
        ...
        allowed_fields = {"title", "scope", "owner", "priority"}
        changes = {k: v for k, v in payload.items() if k in allowed_fields}
        if not changes:
            return {"status": 200, "data": task}

        # S9: once a task is approved (or later), core fields are locked
        if task["status"] in self._LOCKED_STATUSES:
            locked = self._CORE_FIELDS & changes.keys()
            if locked:
                raise ValueError(
                    f"cannot edit {sorted(locked)} on a task with status {task['status']!r}"
                )
    ...
    def _approve_task(self, actor, payload):
        ...
        if not isinstance(task_id, int):
            raise RuntimeError("task_id (int) required")
        task = self._get_task_in_org(task_id, org_id)
        # Existence-hiding: if actor may not approve, respond as if not found
        if not self._can_approve(actor, task):
            raise LookupError(f"task {task_id} not found")
        if task["status"] != "submitted":
            raise ValueError(f"task must be 'submitted' to approve, is {task['status']!r}")
        updated = self._db.update("tasks", task_id, {"status": "approved"})
        self._audit(task_id, org_id, actor.get("user_id", 0), "approve")
        self._invalidate_dashboard(org_id)
        return {"status": 200, "data": updated}
    ...
    def _reject_task(self, actor, payload):
        ...
        if not isinstance(task_id, int):
            raise RuntimeError("task_id (int) required")
        task = self._get_task_in_org(task_id, org_id)
        # Existence-hiding: if actor may not reject, respond as if not found
        if not self._can_approve(actor, task):
            raise LookupError(f"task {task_id} not found")
        if task["status"] != "submitted":
            raise ValueError(f"task must be 'submitted' to reject, is {task['status']!r}")
        updated = self._db.update("tasks", task_id, {"status": "rejected"})
        self._audit(task_id, org_id, actor.get("user_id", 0), "reject")
        self._invalidate_dashboard(org_id)
        return {"status": 200, "data": updated}
    ...
    def _schedule_task(self, actor, payload):
        ...
        if not isinstance(slot, int):
            raise RuntimeError("slot (int) required")
        task = self._get_task_in_org(task_id, org_id)
        if task["status"] != "approved":
            raise ValueError(f"task must be approved to schedule, is {task['status']!r}")
        # conflict check: no two tasks in the same project may share a slot
        project_id = task["project_id"]
        existing = self._db.query("tasks", project_id=project_id, slot=slot)
        if existing:
            raise ValueError(f"slot {slot!r} already taken in project {project_id}")
    ...
    def _search_tasks(self, actor, payload):
        ...
        # Coerce project_id to int if it arrived as a string (e.g. from query params)
        if project_id is not None:
            try:
                project_id = int(project_id)
            except (ValueError, TypeError):
                return {"status": 400, "error": "project_id must be an integer"}

        # All results are always scoped to the actor's org — never leaks cross-org tasks
        tasks = self._db.query("tasks", org_id=org_id)
        ...
        if q:
            # scope may be None (nullable field) — treat None as empty string for matching
            tasks = [t for t in tasks
                     if q in t["title"].lower()
                     or q in (t["scope"] or "").lower()]
...
def render(view: str, backend: Backend, client_state: dict) -> dict:
    ...
    if view == "task_detail":
        ...
        # Compute valid transition actions for the current task status
        available_actions: list[str] = []
        locked_fields: list[str] = []
        if task is not None:
            status = task.get("status")
            can_approve = backend._can_approve(actor, task)
            if status == "draft":
                available_actions = ["submit_task"]
            elif status == "submitted":
                if can_approve:
                    available_actions = ["approve_task", "reject_task"]
            elif status == "approved":
                if can_approve:
                    available_actions = ["schedule_task"]
            # rejected, scheduled, completed → no further transitions

            # S9: show locked core fields for approved-or-later tasks
            if status in Backend._LOCKED_STATUSES:
                locked_fields = sorted(Backend._CORE_FIELDS)
