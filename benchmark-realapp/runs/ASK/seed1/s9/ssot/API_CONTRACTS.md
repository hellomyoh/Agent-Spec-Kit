...
- **S9 lock policy**: if task status is `approved`, `scheduled`, or `completed`, edits to
  `title`, `scope`, or `owner` are refused with **409 Conflict**. `priority` may still be
  changed at any status. Draft and submitted tasks allow all four fields to be edited.
...
Returns `{view, task, comments, available_actions, locked_fields, error}`.
`locked_fields` is `["owner", "scope", "title"]` when status is approved/scheduled/completed,
else `[]`.
