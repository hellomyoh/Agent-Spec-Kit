...
## S8 — Search & filter (completed 2026-06-21)

### Implemented
- `_search_tasks` endpoint: org-scoped task search with optional filters:
  - `q`: case-insensitive substring match against `title` OR `scope` (both checked).
  - `status`: exact match filter on task status.
  - `project_id`: filter to a specific project (must be int; string coercion applied for
    query_param forwarding from render).
- All results always scoped to `actor.org_id` first (`db.query("tasks", org_id=org_id)`);
  filters are applied as in-memory post-processing. Cross-org tasks can never appear.
- Bug fixed: `scope` is nullable (None); `(t["scope"] or "").lower()` prevents
  AttributeError when scope is None.
- Bug fixed: `project_id` arriving as a string from `query_params` (render → search view)
  is coerced to `int`; invalid values return 400.
- `render("search", ...)`: forwards `client_state["query_params"]` as payload to
  `search_tasks`. Already implemented in scaffold; validated in S8.

### Self-test result
All 11 assertions passed:
- q=login title match returns 1 task from org1 only.
- q=frontend matches scope field.
- status=submitted filter returns 1 task.
- project_id filter returns all 3 tasks in project.
- q=auth with None-scope task does not crash; matches both title and scope.
- Org isolation: no org2 tasks leak into org1 search results.
- Case-insensitive matching (LOGIN matches 'Fix the login bug').
- Empty q returns all org tasks (no filtering).
- Combined q + status filters work correctly.
- render("search") view with query_params works end-to-end.
- project_id as string in query_params is coerced to int correctly.

## S9 — Lock core fields after approval (completed 2026-06-21)

### Implemented
- `_update_task`: added post-approval lock policy. When `task["status"]` is in
  `{"approved", "scheduled", "completed"}`, any attempt to edit `title`, `scope`, or `owner`
  raises `ValueError` → 409 Conflict. The stored values are not modified.
- `priority` remains editable at all statuses (non-core field).
- Draft and submitted tasks: all four fields (`title`, `scope`, `owner`, `priority`) still editable (no behaviour change).
- Two class-level constants added to `Backend`: `_LOCKED_STATUSES` and `_CORE_FIELDS`.
- `render("task_detail", ...)`: now returns `locked_fields: list[str]` key. Contains
  `["owner", "scope", "title"]` for approved/scheduled/completed tasks, `[]` otherwise.

### Self-test result
All 10 assertions passed:
- Core field update on draft succeeds (200).
- Priority update on draft succeeds (200).
- Title update on approved task refused (409); stored value unchanged.
- Scope update on approved task refused (409).
- Owner update on approved task refused (409).
- Priority update on approved task succeeds (200).
- task_detail locked_fields = ["owner","scope","title"] for approved task.
- task_detail locked_fields = [] for draft task.
- Core field update on scheduled task also refused (409).
