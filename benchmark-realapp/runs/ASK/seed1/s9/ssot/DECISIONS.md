...
### D27: core fields locked once task reaches approved-or-later status
`update_task` checks `task["status"]` before applying changes. If the status is
`approved`, `scheduled`, or `completed`, attempts to change `title`, `scope`, or `owner`
raise `ValueError` → 409 Conflict. The stored values are NOT modified.

- Core fields (immutable post-approval): `title`, `scope`, `owner`
- Non-core fields (always editable): `priority`
- Locked statuses: `{"approved", "scheduled", "completed"}` (class-level constant `_LOCKED_STATUSES`)
- Core field set: `{"title", "scope", "owner"}` (class-level constant `_CORE_FIELDS`)

This supersedes the "free-edit" behaviour from S1 for approved-or-later tasks. Draft
and submitted tasks are unaffected — all four fields remain editable.

### D28: task_detail render exposes locked_fields list
`render("task_detail", ...)` now returns a `locked_fields: list[str]` key. For tasks
in `approved`, `scheduled`, or `completed` status, this list is `["owner", "scope", "title"]`
(sorted). For all other statuses (`draft`, `submitted`, `rejected`) the list is `[]`.
This is computed from `Backend._LOCKED_STATUSES` and `Backend._CORE_FIELDS` — always
consistent with the enforcement in `_update_task`.
