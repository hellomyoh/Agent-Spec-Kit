...
class Backend:
    ...
    def _can_approve(self, actor, task):
        """Return True if actor may approve/reject this task.

        Allowed roles:
          - org_admin  (any task within their org)
          - project_owner: actor whose user_id matches the project's owner_id
        """
        role = actor.get("role")
        if role == "org_admin":
            return True
        if role == "project_owner":
            project = self._db.get("projects", task["project_id"])
            if project is not None and project["owner_id"] == actor.get("user_id"):
                return True
        return False
    ...
    def _update_task(self, actor, payload):
        ...
        # Core fields are locked once a task is approved or later (approved/scheduled/completed)
        LOCKED_AFTER_APPROVAL = {"approved", "scheduled", "completed"}
        CORE_FIELDS = {"title", "scope", "owner"}
        if task["status"] in LOCKED_AFTER_APPROVAL:
            attempted_core = CORE_FIELDS & set(payload.keys())
            if attempted_core:
                return self._err(CONFLICT, f"fields {sorted(attempted_core)} are locked once the task is approved")
    ...
    def _approve_task(self, actor, payload):
        ...
        if task_id is None:
            return self._err(BAD_REQUEST, "task_id required")
        task = self._get_task(task_id, org_id)
        # Existence-hiding: if not found OR not allowed, return NOT_FOUND
        if task is None or not self._can_approve(actor, task):
            return self._err(NOT_FOUND, "task not found")
        if task["status"] != "submitted":
            return self._err(CONFLICT, f"task must be 'submitted' to perform this action (current: {task['status']})")
        updated = self._db.update("tasks", task_id, {"status": "approved"})
        self._db.insert("audit", {
            "task_id":     task_id,
            "org_id":      org_id,
            "user_id":     actor.get("user_id", 0),
            "action":      "submitted_to_approved",
    ...
    def _schedule_task(self, actor, payload):
        ...
        # Check slot conflict within the same project (no double-booking per project)
        slot_int = int(slot)
        existing = self._db.query("tasks", project_id=task["project_id"], status="scheduled")
        for t in existing:
            t_slot = t.get("slot")
            if t_slot is not None and int(t_slot) == slot_int:
                return self._err(CONFLICT, f"slot '{slot_int}' already taken in this project")
        updated = self._db.update("tasks", task_id, {"status": "scheduled", "slot": str(slot_int)})
        self._db.insert("audit", {
            "task_id":     task_id,
            "org_id":      org_id,
            "user_id":     actor.get("user_id", 0),
...
def render(view, backend, client_state):
    ...
    if view == "project_list":
        ...
        if task is not None:
            ...
            # can_approve: org_admin always; project_owner if they own the project
            role = actor.get("role")
            ...
            if role == "org_admin":
                ...
                projects = proj.get("data") or []
                project_ids_owned = {p["id"] for p in projects if p.get("owner_id") == actor.get("user_id")}
                can_approve_reject = task.get("project_id") in project_ids_owned

            is_privileged = actor.get("role") in ("org_admin", "approver")
            if status == "draft":
                available_actions.append("submit_task")
            elif status == "submitted":
                if can_approve_reject:
                    available_actions.append("approve_task")
                    available_actions.append("reject_task")
            elif status == "approved":
                if is_privileged:
                    available_actions.append("schedule_task")

        # Indicate which fields are locked for UI rendering
        LOCKED_AFTER_APPROVAL = {"approved", "scheduled", "completed"}
        locked_fields = []
        if task is not None and task.get("status") in LOCKED_AFTER_APPROVAL:
            locked_fields = ["title", "scope", "owner"]
