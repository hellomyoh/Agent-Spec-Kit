...
| ASK-team | ASK-solo + 실제 서브에이전트 검토/QA 역할 | team 검토 추가 효과 |
...
- `ASK-team`은 `ASK-solo`보다 설계 결함 또는 hidden failure를 줄여야 한다.
...
ASK-team < ASK-solo 의 design-review-related hidden failures
```
...
4. 변별이 생기면 L 규모와 ASK-team ablation으로 확장한다.
5. 변별이 계속 없으면 "ASK-as-agent-memory의 효과는 약하다"는 결론을 받아들인다.
