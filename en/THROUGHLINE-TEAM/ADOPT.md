# ADOPT.md — Adopting an existing project (THROUGHLINE Team)

Apply THROUGHLINE Team to a project that already has code. Rather than from requirements, **analyze the existing code to reverse-document the current state** and set up a concurrent team-development structure.
...
> For the **detailed method** of code reverse-documentation (state the range read·as-built spec·mark estimates), follow the solo kit ADOPT.md ([reference/SOLO-ADOPT.md](reference/SOLO-ADOPT.md) — bundled reference copy; on conflict the team kit wins) — here we specify **only the team-structure differences**.
...
1. If there are existing artifacts in `THROUGHLINE/`, it's **already adopted** → don't re-adopt, just report.
...
* Adoption artifacts committed (documentation-only; on the shared branch, or merged via PR before claiming starts)
