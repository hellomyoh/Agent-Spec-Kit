...
> all framework documents live **inside the `THROUGHLINE/` folder**. In this document, all artifact paths such as
> ARCHITECTURE.md, PLAN.md, SOURCES/, features/, etc. are relative to `THROUGHLINE/`. Do not create artifacts outside THROUGHLINE/.
...
2. THROUGHLINE/ARCHITECTURE.md  (cross-cutting contracts such as the data model, naming, API contract, auth model, etc.)
3. THROUGHLINE/PLAN.md
4. THROUGHLINE/PROGRESS.md
5. THROUGHLINE/SOURCES/INDEX.md  (status check for unprocessed change requests — read the originals only when processing; see "Important" below)
