...
> **Path baseline (important):** All documents created and managed by this framework go **inside the `THROUGHLINE/` folder** at the project root.
> In this document, all artifact paths such as ARCHITECTURE.md, PLAN.md, SOURCES/, features/, etc. are relative to `THROUGHLINE/`.
...
> To avoid conflicts with an existing project's folders (docs/, etc.), do not create artifacts outside THROUGHLINE/ other than the 3 root files.
...
└── THROUGHLINE/             # ★ Everything owned and managed by the framework
...
* The root `README.md` is the **project README** (artifact). It is a separate document residing in a different repository from the THROUGHLINE repository's guide `README.md`.
* `AGENTS.md` and `CLAUDE.md` **must be created at the project root** (auto-recognition convention). The artifact paths inside these two files are specified with the `THROUGHLINE/` prefix.
* All other documents and folders are created under `THROUGHLINE/`. Do not touch an existing project's folders of the same name (docs/, etc.).
...
* Initialization ends with **a commit**: when the work order above is done (right before the completion report), bundle all generated artifacts (the three root files + THROUGHLINE/) into a **single commit** (Conventional Commits format, e.g., `chore: initialize project with THROUGHLINE`).
...
It is a separate document residing in a different repository from the THROUGHLINE repository's framework guide `README.md`.
...
**all artifact paths inside the document are specified with the `THROUGHLINE/` prefix**.
...
are all inside the THROUGHLINE/ folder.
At the start of work, the Agent first reads AGENTS.md,
always reads ARCHITECTURE.md, PLAN.md, and PROGRESS.md in THROUGHLINE/ together,
checks THROUGHLINE/SOURCES/INDEX.md for unprocessed change requests (Not applied / Under review),
and then selectively reads only the feature documents needed for the current work.
When selecting a feature document, check THROUGHLINE/adr/INDEX.md to see whether there is a related ADR.
```
...
Important design decisions are separated into ADRs, and THROUGHLINE/adr/INDEX.md is updated.
When referencing another document, do not just write the document name; write a relative-path markdown link.
(e.g., in AGENTS.md: [ADR-001](THROUGHLINE/adr/001-auth.md); between documents inside THROUGHLINE/: [ADR-001](../adr/001-auth.md))
Indexes (features/README.md, docs/README.md, adr/INDEX.md — all inside THROUGHLINE/)
...
```text
QA is performed based on the test scenarios in feature documents and the checklists in the THROUGHLINE/qa/ folder.
Per-feature tests are recorded in THROUGHLINE/features/*.md,
and the overall regression tests and release acceptance are managed in the THROUGHLINE/qa/ folder.
"Test passed" is recognized only when the test actually ran and its result was recorded in THROUGHLINE/HISTORY.md.
...
Matters that can be resolved within the design intent of the existing AGENTS.md and THROUGHLINE/'s ARCHITECTURE.md, PLAN.md, PROGRESS.md, features/*.md, qa/*.md
...
```text
Before recording a new assumption, check whether it conflicts with existing items in THROUGHLINE/ASSUMPTIONS.md and with THROUGHLINE/ARCHITECTURE.md.
...
(the actual behavior of an external API, a cause confirmed through debugging, performance characteristics, environment pitfalls) are recorded in THROUGHLINE/NOTES.md.
Speculation is recorded in THROUGHLINE/ASSUMPTIONS.md, not in NOTES.md. Write only confirmed facts in NOTES.md.
...
```text
The narrative prose of artifact documents is written in the primary language of THROUGHLINE/SOURCES/REQUIREMENTS.md.
Code identifiers, API paths, code blocks, technical proper nouns, and commit messages remain in English (no forced translation).
...
```text
PROGRESS.md and HISTORY.md in THROUGHLINE/ record only work that affects code and
...
- In a single commit, atomically bundle the code change together with its corresponding document change (THROUGHLINE/'s features/ARCHITECTURE/PROGRESS/HISTORY, etc.).
...
**Create it at the project root** (moving it into THROUGHLINE/ breaks auto-load).
...
Framework documents, except the 3 root files (README.md/AGENTS.md/CLAUDE.md), are all inside THROUGHLINE/.

## Malfunction prevention (to be observed even when AGENTS.md was not read)

- Do not create artifacts outside THROUGHLINE/.
...
- Do not modify the THROUGHLINE/SOURCES/ originals or the discussion/ logs (immutable).
...
* Project README.md created (separate from the THROUGHLINE guide README.md, no sensitive information)
* PLAN.md created
