# KICKOFF.md — Team project initialization (THROUGHLINE Team)
...
Except for the root 3 files (project `README.md`·`AGENTS.md`·`CLAUDE.md`), create everything under `THROUGHLINE/`.
...
* Initialization ends with **a commit** bundling all generated artifacts (the three root files + THROUGHLINE/), pushed if a remote exists — contributors can only see the coordination layer once it is on the shared branch, so **claims cannot start before this commit**.
