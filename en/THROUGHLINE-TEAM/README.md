...
> It is the **sister framework** of the THROUGHLINE Solo edition (one developer, sequential). Reference copies of the solo sections the team prompts cite are bundled in [reference/](reference/README.md).
...
## 0. Positioning — the sister of the Solo edition

The existing THROUGHLINE (`THROUGHLINE/`) operates around **global single files** like `PLAN.md`·`PROGRESS.md`·`HISTORY.md`·`ASSUMPTIONS.md`·`NOTES.md`·`SOURCES/INDEX.md` for **solo/sequential·autonomous development**. It's simple, but when N people work concurrently these files become conflict hotspots.

THROUGHLINE Team is a separate framework that, **rather than introducing tiers gradually**, fixes every mechanism team-first from the start. For solo development, the existing THROUGHLINE is lighter. THROUGHLINE Team is for **teams where N people actually develop concurrently**.
...
Every design decision in THROUGHLINE Team flows from one fact.
...
When applied to a project, every artifact is created under `THROUGHLINE/` (root 3-file exception: project `README.md`·`AGENTS.md`·`CLAUDE.md`). Same root convention as the existing THROUGHLINE.
...
1. Clone this repository and copy the contents of `en/THROUGHLINE-TEAM/` into the project root's `THROUGHLINE/`.
2. For a **new project**, write the initial requirements in `THROUGHLINE/SOURCES/REQUIREMENTS.md` before running KICKOFF — this kit ships no template, so reuse the solo kit's `SOURCES/REQUIREMENTS.md` template or write the purpose / target users / MVP features / cross-cutting baseline freeform. For an **existing codebase** (ADOPT) it is optional.
...
Each prompt is run by telling your agent to read the file and follow it (e.g., "Read THROUGHLINE/KICKOFF.md and initialize the project").
...
## 9. Differences from the Solo edition

| Item | the Solo edition | THROUGHLINE Team |
|---|---|---|
