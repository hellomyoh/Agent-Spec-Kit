...
2. Check the spec in `features/*.md` and the contracts in `ARCHITECTURE.md` (if absent, start from the spec; review non-trivial features via `personas/`+`discussion/` — the solo ASK DEVELOPINIT §6 way, [reference/SOLO-DEVELOPINIT.md](reference/SOLO-DEVELOPINIT.md)).
