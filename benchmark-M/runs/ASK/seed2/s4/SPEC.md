...
| `pow` | 2 | Exponentiation | `(pow 2 5)` → 32 |
| `neg` | 1 | Unary negation | `(neg 5)` → -5 |
| `lt` | 2 | Less than comparison | `(lt 3 4)` → 1 |
| `gt` | 2 | Greater than comparison | `(gt 3 4)` → 0 |
| `eq` | 2 | Equality comparison | `(eq 4 4)` → 1 |
...
4. **Comparison Operators**: The `lt`, `gt`, and `eq` operators return `1` for true and `0` for false. Examples: `(lt 3 4)` → `1`, `(gt 3 4)` → `0`, `(eq 4 4)` → `1`.
5. **Error Handling**: Any malformed input (unbalanced parens, invalid tokens, wrong operand count) raises `ValueError` with a descriptive message. Division by zero raises `ZeroDivisionError`.
6. **RPN Output**: Returns list of string tokens for compatibility with stack-based evaluation.
7. **Modular Design**: Implementation split across multiple modules (parser, evaluator, rpn) for clarity and maintainability.
