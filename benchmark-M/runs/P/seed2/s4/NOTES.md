...
Extended exprkit package with binary comparison operators `lt`, `gt`, `eq`.

## Changes Made
- Updated `tokenize()` in parser.py to recognize `lt`, `gt`, `eq` tokens
- Updated operator validation in parser.py to accept `lt`, `gt`, `eq` as binary operators (exactly 2 arguments)
- Added comparison handlers in evaluator.py (return 1 for true, 0 for false)
- RPN conversion works without modification (generic operator handling)

## Supported Operators (Session 4)
- Binary `+` (addition)
...
- Binary `pow` (exponentiation, takes exactly 2 arguments)
- Binary `lt` (less than, returns 1 or 0)
- Binary `gt` (greater than, returns 1 or 0)
- Binary `eq` (equal, returns 1 or 0)
...
- `to_rpn("(gt 5 2)") == ["5","2","gt"]` ✓
- `to_rpn("(eq 3 3)") == ["3","3","eq"]` ✓
- All prior operators (+, -, *, /, neg, pow) continue to work in both evaluate and to_rpn ✓
...
## Implementation Complete
Extended exprkit package with binary `pow` operator for exponentiation.

## Changes Made
- Updated `tokenize()` in parser.py to recognize `pow` token
- Updated operator validation in parser.py to accept `pow` as binary operator (exactly 2 arguments)
- Added power handler in evaluator.py (returns `results[0] ** results[1]`)
- RPN conversion works without modification (generic operator handling)

## Supported Operators (Session 3)
- Binary `+` (addition)
- Binary `-` (subtraction, left-associative)
- Binary `*` (multiplication, left-associative)
- Binary `/` (integer floor division, left-associative, raises ZeroDivisionError)
...
- All prior operators (+, -, *, /, neg) continue to work in both evaluate and to_rpn ✓
