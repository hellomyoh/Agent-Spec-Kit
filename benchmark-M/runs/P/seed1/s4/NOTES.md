...
## Overview
Extended the `exprkit` package to support the binary comparison operators `lt`, `gt`, and `eq`. These operators return 1 (true) or 0 (false) when comparing two values. All prior operators (`+`, `-`, `*`, `/`, `neg`, `pow`) continue working in both `evaluate` and `to_rpn`.
...
- **parser.py**: Tokenizes and parses s-expressions into an AST
  - `tokenize()`: Splits input string into tokens (parentheses, operators, integers)
    - Updated regex to recognize `lt`, `gt`, `eq` as keyword operators
  - `parse()`: Builds an abstract syntax tree (dict-based AST)
  - Validates operators including `lt`, `gt`, `eq` with appropriate argument counts
...
- **evaluator.py**: Evaluates the AST to compute results
  ...
  - `eval_ast()`: Recursively evaluates AST nodes
    ...
    - `lt`, `gt`, `eq` (binary) require exactly 2 arguments and return 1 or 0
...
- **rpn.py**: Converts AST to postfix notation
  ...
  - `ast_to_rpn()`: Recursively builds RPN token list
    ...
    - `lt`, `gt`, `eq` follow the binary operator pattern
...
- Supports binary `lt`, `gt`, `eq` comparison operators
- `/` uses integer floor division (returning int): `(/ 7 2)` → 3, `(/ -7 2)` → -4
- `pow` uses Python's `**` operator: `(pow 2 5)` → 32, `(pow (+ 1 1) (- 4 1))` → 8
- `lt`, `gt`, `eq` return 1 (true) or 0 (false): `(lt 3 4)` → 1, `(eq 4 4)` → 1, `(gt 3 4)` → 0
...
- `(lt 3 4)` → evaluate=1, rpn=["3","4","lt"]
- `(gt 3 4)` → evaluate=0, rpn=["3","4","gt"]
- `(eq 4 4)` → evaluate=1, rpn=["4","4","eq"]
- `(eq (* 2 3) (+ 5 1))` → evaluate=1, rpn=["2","3","*","5","1","+","eq"]
- `(lt 5 10)` → evaluate=1, rpn=["5","10","lt"]
...
## Changes in Session 4
- Updated `parser.py`: Added `lt`, `gt`, `eq` to tokenizer regex and operator validation
- Updated `evaluator.py`: Implemented `lt`, `gt`, `eq` operators with 2 argument check returning 1 or 0
- Updated `rpn.py`: Added `lt`, `gt`, `eq` to operator validation (follows binary op pattern already in place)
- All prior operators (`+`, `-`, `*`, `/`, `neg`, `pow`) continue working in both `evaluate` and `to_rpn`
...
4. Support additional operators (le, ge, ne for comparisons; abs, etc. for unary)
