def evaluate_ast(ast):
    ...
    if op == '+':
        ...
        if len(args) != 2:
            raise ValueError("Less-than requires exactly 2 operands")
        left = evaluate_ast(args[0])
        right = evaluate_ast(args[1])
        return 1 if left < right else 0
    elif op == 'gt':
        if len(args) != 2:
            raise ValueError("Greater-than requires exactly 2 operands")
        left = evaluate_ast(args[0])
        right = evaluate_ast(args[1])
        return 1 if left > right else 0
    elif op == 'eq':
        if len(args) != 2:
            raise ValueError("Equality requires exactly 2 operands")
        left = evaluate_ast(args[0])
        right = evaluate_ast(args[1])
        return 1 if left == right else 0
    else:
