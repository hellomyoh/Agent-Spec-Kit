...
def tokenize(s: str):
    ...
    while i < len(s):
        if s[i].isspace():
            ...
            tokens.append('pow')
            i += 3
        elif s[i:i+2] == 'lt' and (i+2 >= len(s) or s[i+2].isspace() or s[i+2] in '()'):
            # Keyword operator 'lt'
            tokens.append('lt')
            i += 2
        elif s[i:i+2] == 'gt' and (i+2 >= len(s) or s[i+2].isspace() or s[i+2] in '()'):
            # Keyword operator 'gt'
            tokens.append('gt')
            i += 2
        elif s[i:i+2] == 'eq' and (i+2 >= len(s) or s[i+2].isspace() or s[i+2] in '()'):
            # Keyword operator 'eq'
            tokens.append('eq')
            i += 2
