...
## D6 (S1) — `where` equality filter + unknown-field policy [SUPERSEDED by D10 (S5)]
...
- **SUPERSEDED (S5/D10):** the unknown-field-raises-`KeyError` rule (and its
  empty-store corollary) is REVERSED — an unknown `where` field now yields NO match
  (returns `[]`), never raising. The non-superseded parts of D6 still hold: the
  AND/`==` matching, empty/None `where` no-op, and "a record lacking a present key
  is filtered out (not an error)". See D10.
...
## D10 (S5) — `where` unknown field → no match (SUPERSEDES D6's raise rule) [REVERTED by D11 (S6)]
Ticket S5 CHANGES the unknown-field behaviour of `where`. New policy:
- A `where` key that is present in NO stored record now yields NO match: such a
  query returns `[]`. It MUST NOT raise. (Reverses the D6 `KeyError(key)` rule.)
- Corollary: on an EMPTY store, any non-empty `where` returns `[]` (was: raised).
- Implementation: the unknown-field pre-validation loop (and the `_known_keys()`
  check it used for `where`) is REMOVED. The existing match predicate
  `all(k in r and r[k] == v ...)` already produces no match when a key is absent
  from every record, so dropping the guard is sufficient — no new code path.
- UNCHANGED from D6 (still in force): AND across all key==value pairs, `==`
  comparison on the stored value, insertion order, copies returned, empty/None
  `where` is a no-op, and a record that merely LACKS one of the `where` keys (while
  other records have it) is filtered out (not matched, not an error).
- SCOPE: this affects ONLY `where`. `sort_by`'s fail-fast unknown-field rule (D7)
  is deliberately LEFT UNCHANGED — the ticket changed `where` alone. So `where` and
  `sort_by` now diverge on unknown fields (`where` → `[]`; `sort_by` → `KeyError`),
  and `select` continues not to validate (D9).
- Rationale: ticket-mandated. Treats `where` as a pure filter (an unmatched
  predicate is empty, not erroneous) rather than a schema check.
- **REVERTED (S6/D11):** this relaxation is undone — an unknown `where` field
  again raises `KeyError` (and the empty-store corollary again raises). The
  non-reverted parts (AND/`==`, empty/None no-op, record-lacking-a-present-key
  filtered out) still hold, exactly as in D6. See D11.

## D11 (S6) — REVERT S5: `where` unknown field → `KeyError` again (restores D6)
Ticket S6 is a pure revert of S5/D10. It restores the unknown-field behaviour of
`where` that was in force BEFORE S5 (the S1/D6 rule):
- A `where` key present in NO stored record (not in the union of keys across all
  records) again raises `KeyError(key)` — a hard error, not a silent no-match.
- Corollary RESTORED: on an EMPTY store, any non-empty `where` raises `KeyError`
  (no record contains the key). (S5/D10 had made both cases return `[]`.)
- Implementation: the unknown-field pre-validation loop that D10 had removed is
  reinstated in the `where` branch — for each key in `where`, if it is not in
  `_known_keys()`, raise `KeyError(k)`. The match predicate
  (`all(k in r and r[k] == v ...)`) is retained for actual filtering.
- UNCHANGED (still in force, exactly as D6 specified): AND across all key==value
  pairs, `==` comparison on the stored value, insertion order, copies returned,
  empty/None `where` is a no-op, and a record that merely LACKS one of the
  (otherwise-present) `where` keys is filtered out (not matched, not an error).
- SCOPE: this affects ONLY `where`. `sort_by`'s fail-fast unknown-field rule (D7)
  was never changed and remains as-is; `select` continues not to validate (D9).
  With this revert, `where` and `sort_by` are once again CONSISTENT on unknown
  fields (both raise `KeyError`); they had diverged under D10.
- DECISION HISTORY for `where` unknown-field handling (net effect):
  D6 (S1) raise → D10 (S5) return `[]` → D11 (S6) raise again. Current rule = D6.
- Rationale: ticket-mandated revert. Restores fail-fast on typo'd/absent `where`
  field names, matching the S1 contract and re-aligning with `sort_by` (D7).
