...
Implemented so far:
  ...
  - Store.query()       : return matching records (copies).
      * where={k: v, ...} : keep records matching ALL key==value pairs (S1;
                            unknown-field handling changed in S5).
...
class Store:
    ...
    def query(
        ...
        Policy (S5/D10, SUPERSEDES S1/D6): a ``where`` key that is not present
        in ANY stored record is NOT an error — it simply matches no record, so
        ``query()`` returns ``[]``. (Previously this raised ``KeyError``.) A
        record that merely lacks a key while other records have it is likewise
        filtered out, not matched. On an EMPTY store, any non-empty ``where``
        returns ``[]``.
        ...
        sort position is undefined. (Note: unlike ``where``, ``sort_by`` retains
        its fail-fast unknown-field rule — S5 changed only ``where``.)
        ...
        if where:
            # Unknown-field policy (S5/D10, supersedes D6): a `where` key absent
            # from every record is NOT an error. The match predicate below
            # already yields no match for such a key (no record contains it),
            # so query() returns []. No pre-validation is performed.
            results = [
                r
                for r in results
        ...
        if sort_by is not None:
            ...
            # store, else it is a hard error. (S5 left this `sort_by` rule
            # unchanged; only `where` was relaxed.)
