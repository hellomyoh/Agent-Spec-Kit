...
SUPERSEDED by D9 [S5]: the default page size value is now `40`. The mechanism
(omitted-`limit` cap via the `DEFAULT_PAGE_SIZE` constant) is unchanged.

## D9 [S5] — DEFAULT PAGE SIZE = 40 (supersedes D7)
When `limit` is omitted (`None`), `query()` returns at most `40` records. The module
constant is now `DEFAULT_PAGE_SIZE = 40`. This changes only the value (previously `25`
in D7, `7` in D5); the policy is otherwise identical — it governs only the
omitted-`limit` case, and an explicit `limit` (including `0`) overrides it.
SUPERSEDED by D10 [S6]: the default page size is restored to its original `7`. The
mechanism (omitted-`limit` cap via the `DEFAULT_PAGE_SIZE` constant) is unchanged.

## D10 [S6] — DEFAULT PAGE SIZE restored to 7 (supersedes D9)
Restores the default page size to `7`, the value it was originally given when first
introduced in S1 (D5), undoing the later adjustments to `25` (D7) and `40` (D9). The
module constant is now `DEFAULT_PAGE_SIZE = 7`. This changes only the value; the policy
is otherwise identical — it governs only the omitted-`limit` case, and an explicit
`limit` (including `0`) overrides it. Value lineage: 7 (D5) -> 25 (D7) -> 40 (D9) -> 7 (D10).
