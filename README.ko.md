...
- **ASK Solo/Team 프롬프트 보강** (KICKOFF/AUDIT/DEVELOPINIT, `en/`·`ko/` 모두) — 프롬프트 리뷰에서 드러난 네 가지 빈틈을 반영:
  - **PLAN.md 로테이션 규칙** (KICKOFF 12) + AUDIT 3.9/4를 실행 가능하게 정리 — 기계적 PLAN 아카이브 대 관찰 전용 ARCHITECTURE — KICKOFF↔AUDIT 모순 해소.
  - **라이브 대화 대 SOURCES 권위** (DEVELOPINIT 4.2.1) — 대화 지시는 그 지시가 서술한 변경을 승인하는 것이지 휴면 상태의 SOURCES 문서를 승인하는 게 아님; 핵심/횡단 변경은 여전히 산출물 경로를 거침.
  - **커밋/푸시 기본값을 커밋 전용으로 완화** — 푸시는 AGENTS.md 푸시 정책을 따름(KICKOFF 10 템플릿 + DEVELOPINIT + README 프롬프트 블록). main/master/force/secret 가드는 유지.
  - **멀티에이전트 리뷰 실행 모드 정직성** (KICKOFF 4.1 로그에 실행 모드 필드 + "병렬 리뷰 위장 금지"; AUDIT 3.10 표본 점검; README 5.3 폴백 / 9.2 뉘앙스), 페르소나별 검증 가능 실패 조건 조항 포함.

### [2026-06-29]

- **벤치마크 근거 추가** (`benchmark/benchmark-solo-pilot/`, `benchmark/benchmark-vibe-ask-solo/`) 및 두 건의 단일 시드 드리프트 억제 파일럿을 정리한 [부록: 벤치마크 근거](#부록-벤치마크-근거-드리프트-억제-파일럿) — 메모리 보존(초기 결정이 메모리 산출물 안에만 살아남음)과 바이브 코딩 드리프트(SSOT + 충돌 점검 대 일반 진행을 beginner/intermediate/advanced 프롬프트 레벨에 걸쳐 비교). 검정력 있는 결과가 아닌 정직한 go/no-go 판별 검사. 문서만 변경했고 키트 프롬프트는 손대지 않음.
