...
- **벤치마크 근거 추가** (`benchmark-solo-pilot/`, `benchmark-vibe-ask-solo/`) 및 두 건의 단일 시드 드리프트 억제 파일럿을 정리한 [부록: 벤치마크 근거](#부록-벤치마크-근거-드리프트-억제-파일럿) — 메모리 보존(초기 결정이 메모리 산출물 안에만 살아남음)과 바이브 코딩 드리프트(SSOT + 충돌 점검 대 일반 진행을 beginner/intermediate/advanced 프롬프트 레벨에 걸쳐 비교). 검정력 있는 결과가 아닌 정직한 go/no-go 판별 검사. 문서만 변경했고 키트 프롬프트는 손대지 않음.
...
### 파일럿 1 — `benchmark-solo-pilot/`: 초기 결정의 기억은 살아남는가?
...
### 파일럿 2 — `benchmark-vibe-ask-solo/`: SSOT는 바이브 코딩 드리프트를 억제하는가?
...
> 전체 설계·원시 트래젝토리·go/no-go 판정: `benchmark-solo-pilot/RESULTS_v2.md`, `benchmark-vibe-ask-solo/RESULTS_seed1.md`.
