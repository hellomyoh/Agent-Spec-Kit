...
명세 주도 개발을 하려고 프롬프트를 붙여넣어야 하는 것은 아닙니다. 초기화가 프로젝트 루트의 `AGENTS.md`에 상시 규약을 써두고(`CLAUDE.md`는 안전망), 에이전트가 **매 실행마다** 그것을 로드합니다 — 그래서 Claude Code · Codex · Cursor와 평범하게 대화만 해도 SSOT(`ARCHITECTURE.md` · `PLAN.md` · `PROGRESS.md`)를 읽고, 새 요청을 기록된 결정과 대조하고, 바뀐 것을 기록합니다. 채팅 지시가 영향도에 따라 어떻게 라우팅되는지는 [5.1.1절](#511-대화로-바로-수정추가하기-라이브-채팅)에 있습니다.

[벤치마크](#벤치마크)가 측정한 것이 실은 이 대화 모드입니다 — 세션마다 개발 에이전트에게 상시 지시문과 평범한 사용자 요청만 주었고, **THROUGHLINE 프롬프트는 붙여넣지 않았습니다**.
