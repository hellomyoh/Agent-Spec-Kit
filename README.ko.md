...
**어느 쪽도 프롬프트를 붙여넣지 않았습니다.** 세션마다 개발 에이전트에게 상시
지시문과 평범한 사용자 요청만 주었습니다 — 즉 +24.0은 사용자가 성실히 프롬프트를
써서 나온 값이 아니라 **상시 규약만으로** 나온 값입니다.
...
2. 여러분 **언어의 `THROUGHLINE/` 폴더** — `ko/THROUGHLINE/` 또는 `en/THROUGHLINE/` — 를 프로젝트 루트로 복사합니다. (이 가이드는 복사하지 않습니다.)
3. **신규 프로젝트**면 `THROUGHLINE/SOURCES/REQUIREMENTS.md`에 요구사항을 작성합니다. **이미 코드가 있으면** 건너뜁니다.
4. 프로젝트에서 Agent를 열고 **[초기화 프롬프트](#2-최초-1회-프로젝트-초기화-프롬프트)**(기존 코드는 **[채택 프롬프트](#21-이미-개발-중인-프로젝트에-적용할-때-채택-프롬프트)**)를 붙여넣은 뒤, **[개발 프롬프트](#5-실제-개발-시작-프롬프트)**로 개발을 시작합니다.

> 모든 산출물은 `THROUGHLINE/` 안에 생성되어 기존 프로젝트 폴더(docs/ 등)와 충돌하지 않습니다. 루트에는 `AGENTS.md`·`CLAUDE.md`와 프로젝트 `README.md`만 생성/병합됩니다.

초기화 이후에는 `AGENTS.md`가 매 실행마다 로드되므로, 더 붙여넣지 않고 평범한 대화만으로도 이어갈 수 있습니다 → [프롬프트 없이 쓰기](#프롬프트-없이-쓰기).
...
명세 주도 개발을 하려고 프롬프트를 붙여넣어야 하는 것은 아닙니다. 초기화가 프로젝트 루트의 `AGENTS.md`에 상시 규약을 써두고(`CLAUDE.md`는 안전망), 에이전트가 **매 실행마다** 그것을 로드합니다 — 그래서 Claude Code · Codex · Cursor와 평범하게 대화만 해도 SSOT(`ARCHITECTURE.md` · `PLAN.md` · `PROGRESS.md`)를 읽고, 새 요청을 기록된 결정과 대조하고, 바뀐 것을 기록합니다. 채팅 지시가 영향도에 따라 어떻게 라우팅되는지는 [5.1.1절](#511-대화로-바로-수정추가하기-라이브-채팅)에 있습니다. [벤치마크](#벤치마크)를 돌린 것도 이 모드입니다.

프롬프트를 붙여넣는 것은 기본값이 아니라 *보장*을 사는 일입니다. 대화만 할 때 에이전트의 판단에 기대게 되는 것이 둘 있고, 중요한 상황이라면 프롬프트를 쓰는 편이 낫습니다: **비자명 명세 전의 페르소나 검토**([5.3절](#53-기능-추가-검토설계-프롬프트-검토-강도-선택))와 **세션 종료 시 `PROGRESS.md` 인계 기록**([7절](#7-다음-날-또는-세션이-끊긴-뒤-이어서-작업하는-프롬프트)). 빠져나간 것을 주기적으로 걷어내는 그물은 [`AUDIT.md`](#91-주기적-문서-감사-auditmd)입니다.
...
### 5.1.1 대화로 바로 수정/추가하기 (라이브 채팅)

항상 문서를 써야 하는 것은 아닙니다. 에이전트에게 채팅으로 말해서 변경을 요청할 수도 있으며, 프레임워크는 영향 범위로 처리 경로를 나눕니다(`DEVELOPINIT.md` 4.2.1).

- **작은/비핵심 변경**(문구, 비핵심 UI, 내부 수정): 채팅으로 말하면 에이전트가 변경하고 그 결정을 `ASSUMPTIONS.md`/`HISTORY.md`에 기록합니다. 문서가 필요 없습니다.
- **핵심/횡단 변경**(MVP 범위, 데이터 모델, 인증/권한, `ARCHITECTURE.md` 계약): 채팅 지시는 작업을 *시작*시키지만 기록을 생략하게 해 주지는 **않습니다.** 에이전트는 먼저 확인한 뒤 같은 작업 안에서 변경을 산출물(ARCHITECTURE/features/ADR)에 반영하며, 그 이후로는 채팅 메시지가 아니라 **산출물**이 진실의 출처가 됩니다.
- **추적성이 필요할 때**("몇 달 전에 왜 이걸 바꿨지?") 또는 변경이 크거나 논쟁적일 때: 5.1절의 문서 채널을 권장합니다. 제출된 원본이 보존되므로 이유가 남습니다.

> **권위 주의.** 채팅 지시는 **그 지시가 서술하는 변경**을 승인하는 것이지, 아직 `미반영`/`검토 중`인 `SOURCES/` 변경요청 문서를 활성화하는 것이 아닙니다. 휴면 상태의 요청 문서는 `반영 완료`가 되기 전까지 권위가 없습니다(KICKOFF.md 15.2).
...
[MIT License](LICENSE)로 배포됩니다 — 출처 표시만 하면 사용·복사·수정·배포(상업적 이용 포함)가 모두 자유입니다. Copyright (c) 2026 hellomyoh.
