...
**세션이 끝나도 기억이 이어지고, 하네스 패턴의 의사결정 그룹이 명세를 합의하는 — Agent 기반 프로젝트 초기화·개발 프롬프트 프레임워크.**

Agent가 요구사항을 명세와 계획으로 컴파일하고, 결정·지식을 마크다운 파일 시스템에 축적해 다음 세션이 이어받습니다.
페르소나 의사결정 그룹(하네스 패턴의 프롬프트 구현)이 기능명세를 토론·합의하고, 토의 과정은 검증 가능한 기록(`discussion/`)으로 남습니다.
